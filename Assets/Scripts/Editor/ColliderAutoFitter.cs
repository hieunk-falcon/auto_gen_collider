...
public static class ColliderAutoFitter
{
    ...
    const int   MAX_PARTS      = 12;    // hard cap on primitives per object
    ...
    public static FitResult FitCollider(GameObject root, FitOptions opt)
    {
        ...
        var build = BuildSolid(root, full, opt.modules, centroid, axes);
                                              ...
                                              ModuleLibrary modules, Vector3 centroid, Vector3[] axes)
    {
        ...
        if (TryFindSymmetryPlane(full.verts, centroid, axes, out Vector3 symN))
            return BuildSolidSymmetric(root, full, modules, centroid, symN);

        return BuildSolidPlain(root, full, modules);
    }
                                                       ...
                                                       Vector3 centroid, Vector3 normal)
    {
        ...
        if (half.verts.Length < 4) return BuildSolidPlain(root, full, modules);

        var leaves = DecomposeToLeaves(root, half);
        if (leaves.Count == 0) return BuildSolidPlain(root, full, modules);
        ...
        foreach (var leaf in leaves)
        {
            ...
            if (dmin <= band)
            {
                ...
                vol += FitLeaf(root, MirrorComplete(leaf.verts, centroid, normal), modules, ref idx);
            }
            else
            {
                // Side part + its exact mirror (same deterministic fit → identical).
                vol += FitLeaf(root, leaf.verts, modules, ref idx);
                vol += FitLeaf(root, Reflect(leaf.verts, centroid, normal), modules, ref idx);
            }
        }
    ...
    static (int count, double vol) BuildSolidPlain(GameObject root, MeshCluster full, ModuleLibrary modules)
    {
    ...
    static double FitLeaf(GameObject root, Vector3[] verts, ModuleLibrary modules, ref int idx)
    {
        ComputePCA(verts, out Vector3 c, out Vector3[] ax, out float[] ex);

        if (IsTube(ex))
            return FitCapsuleChain(root, verts, ChainSegments(ex), modules, ref idx);

        double v = MakeColliderChild(root, PREFIX + idx, verts, c, ax, Classify(ex, modules), modules);
        idx++;
        return v;
    }
    ...
    static double FitCapsuleChain(GameObject root, Vector3[] verts, int segments,
                                  ModuleLibrary modules, ref int idx)
    {
        ComputePCA(verts, out Vector3 c, out Vector3[] ax, out _);
        Vector3 axis = ax[0];
        ...
        for (int s = 0; s < segments; s++)
        {
            ...
            {
                float p = Vector3.Dot(v - c, axis);
                if (p >= lo && p <= hi) sub.Add(v);
            }
            if (sub.Count < 4) continue;

            Vector3[] subArr = sub.ToArray();
            ComputePCA(subArr, out Vector3 cc, out Vector3[] sax, out _);
            vol += MakeColliderChild(root, PREFIX + idx, subArr, cc, sax, ShapeType.Capsule, modules);
            idx++; made++;
        }

        if (made == 0)
        {
            vol += MakeColliderChild(root, PREFIX + idx, verts, c, ax, ShapeType.Capsule, modules);
            idx++;
        }
        return vol;
    }
    ...
    static void ValleyDecompose(MeshCluster cluster, int depth, List<MeshCluster> output)
    {
        ...
        ComputePCA(cluster.verts, out _, out _, out float[] gex);
        if (IsTube(gex))
        {
            output.Add(cluster);
            return;
        ...
        if (!FindBestCut(cluster, out Vector3 pp, out Vector3 pn, out float score) ||
            score < MIN_CUT_SCORE)
        {
            output.Add(cluster);
            return;
    ...
    // radius-shoulder. Returns the plane (point + normal) and its 0..1 score.
    static bool FindBestCut(MeshCluster cluster, out Vector3 planePoint,
                            out Vector3 planeNormal, out float score)
    {
        ...
        for (int k = 0; k < 3; k++)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                proj[i] = Vector3.Dot(cluster.verts[i] - centroid, axis);
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                cnt[b]++;
                Vector3 d  = cluster.verts[i] - centroid;
            ...
            if (maxR > 1e-6f)
            {
                for (int i = lo; i < hi; i++)
                {
                    ...
                    float step = Mathf.Abs(rad[i + 1] - rad[i]) / maxR;
                    if (step > score)
                    {
                                    ...
                                    ModuleLibrary modules)
    {
        ...
        switch (shape)
        {
            ...
            {
                var go = NewChild(root, name, centroid, Quaternion.identity);
                float r = 0f;
            ...
            {
                // Length axis = axes[0]; radius = max perpendicular distance.
                Vector3 axis = axes[0];
                float maxR = 0f;
                foreach (var v in verts)
                {
                    Vector3 d = v - centroid;
                    float   pe = (d - Vector3.Dot(d, axis) * axis).magnitude;
                    if (pe > maxR) maxR = pe;
                }
                float h  = Mathf.Max(2f * half[0], 2f * maxR);
                var   go = NewChild(root, name, centroid + axis * mid[0], BuildPCARot(axes));
                var   cc = go.AddComponent<CapsuleCollider>();
                cc.center = Vector3.zero; cc.direction = 1; cc.radius = maxR; cc.height = h;
                double cyl = Mathf.PI * maxR * maxR * Mathf.Max(h - 2f * maxR, 0f);
                double cap = 4.0 / 3.0 * Mathf.PI * maxR * maxR * maxR;
                return cyl + cap;
            }

            case ShapeType.Cylinder:
            {
                Extents(verts, centroid, axes, out _, out float[] ex);
                int     sym = FindSymmetryAxis(ex);
                Vector3 up  = axes[sym];
                ...
                Vector3 pos = centroid + up * mid[sym];
                ...
                if (prefab != null)
                {
                    ...
                    go.transform.localRotation = BuildAxisRot(up, axes[(sym + 1) % 3]);

                    // Normalise by the module's native size so any unit convention
                    // works (e.g. Unity's Cylinder mesh is 2 tall, Ø1). Target:
                    // local X/Z = diameter (2·rad), local Y = full height.
